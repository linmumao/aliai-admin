    ...
    <div id="activitytype">
      ...
      <div id="top">
        <div id="toptop">
          ...
          <div style="margin-top: 10px;">
            <el-button @click="neiactivitylist(dataOne)" type="primary" size="mini">查询</el-button>
      ...
      <div id="middle">
        ...
        <el-table :header-cell-style="{ background: '#f5f7fa', color: '#000' }" border size="small"
          @selection-change="selectChange" :data="listData" highlight-current-row v-loading="loading"
          ...
          <el-table-column prop="followPeople" label="跟进人员" width="100">
          </el-table-column>
          ...
          <el-table-column align="center" label="操作" min-width="350" fixed="right">
            <template slot-scope="scope">
              ...
              <el-button size="mini" type="primary" @click="addData(scope.row)">数据</el-button>
      ...
      <div>
        <el-dialog title="数据上传" :visible.sync="dialogFormVisible" width="550px">
          <div style="display: flex;margin-bottom: 20px; ">
            <span>参与人数：</span>
            <el-input v-model="input" placeholder="请输入内容" style="width: 100px; margin-right: 100px; "></el-input>
            <el-button type="primary" @click="add" plain>点击添加更多字段</el-button>
          </div>
          <div style="display: flex; justify-content: center;">
            <table>
              <tr v-for="(item, index) in shopingCarts">
                <td>
                  <el-input style="width: 150px" v-model="item.key" placeholder="请输入字段名称"></el-input>
                  <span>:</span>
                  <el-input style="width: 150px" v-model="item.price" placeholder="请输入对应的数值"></el-input>
                </td>
                <td>
                  <el-button type="primary" @click="remove(index)" plain>删除</el-button>
                </td>
              </tr>
            </table>
          </div>
...
export default {
  ...
  mounted() {
    this.neiactivitylist(this.dataOne);
  },

  methods: {
    // 根据条件分页查询
    neiactivitylist(dataOne) {
      this.dataOne.pageNo = this.pageNo;
      console.log(this.pageNo);
      console.log(this.currentPage);
      this.dataOne.pageSize = this.pageSize;
      neiactivitylist(dataOne).then((res) => {
        console.log("根据条件分页内部活动列表", res);
        if (res.code == 0) {
    ...
    deleteactivitylist(row) {
      (this.editId = row.id), console.log("row", row);
    ...
    //上传数据
    async uploadneiactivitylist() {
      const customization = {};
      this.shopingCarts.forEach((item) => {
        customization[item.key] = item.price;
      });
      const params = {
        id: this.editDataID,
        inviteNum: 0,
      ...
      try {
        const res = await uploadneiactivitylist(params);
        // 如果情趣数据成功，那么刷新数据
        if (res.code === 0) {
    ...
    add() {
      this.one = [];
      this.shopingCarts.push({ price: "", key: "" });
    },
    //复制跳转到新增页面
    copy(row) {
      this.$router.push({
