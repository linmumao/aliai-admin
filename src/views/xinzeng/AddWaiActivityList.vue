    ...
    <div style="padding: 0 20px;  background-color:#f5f7fa ">
        ...
        <div style="padding-top: 15px;">
            <el-breadcrumb separator-class="el-icon-arrow-right">
                <el-breadcrumb-item class="myColor" :to="{ path: '/' }">活动列表
                </el-breadcrumb-item>
        ...
        <div id="operationlog">
            <div id="mainoperationlog" style="width: 58%; ">
                <div style="font-size: 16px;font-weight: 600;color: black; ">操作日志</div>
                ...
                <div class="asidecss">
                    ...
                    <el-form :model="form" >
                        <el-form-item label="活动名称：" :label-width="formLabelWidth">
                            <el-input v-model="form.activityName" autocomplete="off" style="width: 338px;"  :disabled="disabled" ></el-input>
                        </el-form-item>
                        <el-form-item label="活动类型：" :label-width="formLabelWidth">
                          <el-select v-model="form.activityType" placeholder="请选择" style="width: 340px;" :disabled="disabled">
                                <el-option
                        ...
                        <el-form-item label="所属支行：" :label-width="formLabelWidth">
                            <el-select v-model="form.belongBank" placeholder="请选择" style="width: 340px;" :disabled="disabled">
                                <el-option
                        ...
                        <el-form-item label="活动日期：" :label-width="formLabelWidth" :disabled="disabled">
                            <el-date-picker
                        ...
                        <el-form-item label="活动地点：" :label-width="formLabelWidth">
                            <el-input v-model="form.activityAddress" autocomplete="off" style="width: 338px;" :disabled="disabled"></el-input>
                        </el-form-item>
                        <el-form-item label="备注：" :label-width="formLabelWidth">
                            <textarea name="one" id="one"  v-model="form.remark" placeholder="请输入不多50字的描述"    maxlength="10" :disabled="disabled">
                            </textarea>
                        </el-form-item>
                    </el-form>
                </div>
                <div class="asidecss">
                    ...
                    <el-form :model="form" >
                        <el-form-item label="活动类型名称：" :label-width="formLabelWidth">
                            <el-select v-model="form.activityTypeName" placeholder="请选择" style="width: 340px;" :disabled="disabled">
                                <el-option
                        ...
                        <el-form-item label="当前跟进人员：" :label-width="formLabelWidth">
                            <el-input v-model="form.followPeople" autocomplete="off" style="width: 338px;" :disabled="disabled"></el-input>
                        </el-form-item>
                        <el-form-item label="邀约人数：" :label-width="formLabelWidth">
                            <el-input v-model="form.inviteNum" autocomplete="off" style="width: 338px;" :disabled="disabled"></el-input>
                        </el-form-item>
                        <el-form-item label="初步预估费用：" :label-width="formLabelWidth">
                            <el-input v-model="form.estimatedCost" autocomplete="off" style="width: 338px;" :disabled="disabled"></el-input>
                        </el-form-item>
                    </el-form>
                </div>
                <div class="asidecss">
                    <div style="display: flex; justify-content: space-between;">
                        ...
                        <el-button type="primary" plain  v-show="this.editStatus != -1">全部下载</el-button>
                    </div>
                    <el-form :model="form" >
                        <el-form-item label="活动视频地址：" :label-width="formLabelWidth">
                            ...
                            <el-button type="primary" @click="add" plain>点击添加更多视频地址</el-button>
                        ...
                        <div style="display: flex; justify-content: space-between;">
                            ...
                            <a href="http://192.168.1.74/api/marketing/activitylist/download/img?id=1622864707356594176&fileName=down">下载</a>
                            <!-- <el-button type="primary" plain v-show="this.editStatus != -1" @click="downloadfiles">下载</el-button> -->
                        </div>
                        <div style="display: flex; justify-content: space-between;">
                            ...
                            <el-button type="primary" plain v-show="this.editStatus != -1" @click="downloadpictures">下载</el-button>
                        </div>
                        <div style="display: flex; justify-content: space-between;">
                            <el-form-item label="简短总结：" :label-width="formLabelWidth">
                                <textarea name="one" id="one"  v-model="form.sumUp" placeholder="请输入不多50字的描述"    maxlength="10">
                                </textarea>
                            </el-form-item>
                            <el-button type="primary" plain v-show="this.editStatus != -1" @click="downloadtext">下载</el-button>
                        </div>
                        <el-form-item label="活动实际费用：" :label-width="formLabelWidth">
                            <el-input v-model="form.actualCost" autocomplete="off" style="width: 338px;"></el-input>
                        </el-form-item>
                    </el-form>
                </div>
                <div id="buttomcss">
                    ...
                    <el-button type="primary" v-show="editStatus !== '-1'&&this.form.activityStatus !== '3'" @click="submitForm('ruleForm')">保存</el-button>
                    <el-button type="primary" v-show="editStatus !== '-1'&&this.form.activityStatus === '3'" @click="submitForm('ruleForm')">重新保存</el-button>
                </div>
                <!-- <div id="buttomcss">
...
import {addneiactivitylist ,waiupdataactivitylist,  updataactivitylist ,journal ,findbankbranch ,downloadfiles , downloadpictures , downloadtext } from '../../api/xinzeng'
    ...
    export default {
        ...
        created () {
            ...
            if(this.$route.query.upMangerId == -1){
                ...
                this.form.activityNameOut = this.$route.query.upMangerId.activityNameOut; //内部活动名称

                this.form.activityStatus = this.$route.query.upMangerId.activityStatus; //活动状态
                if(this.form.activityStatus === 3){
                    this.disabled=false
                }
        ...
        methods:{
            ...
            submitForm (formName) {
                ...
                //     console.log(this.form)
                    ...
                    if (this.editStatus == '-1') {
                        for( var i = 0; i < this.belongBank.length; i++){   //处理所属银行id
                        ...
                        waiupdataactivitylist(this.form).then((res) => {
                        console.log("编辑:", res);
                        if (res.code == 0) {
            ...
            beforeAvatarUpload (file) {
                ...
                if (isLt2M && isJPG || isLt2M && isPng || isLt2M && isGif) {
                    ...
                    this.$message.error('上传头像只能是PNG或JPG格式或gif格式,并且不能大于2M');
                }
            ...
            // 批量下载文件
            downloadfiles(){
                downloadfiles(this.editStatus).then(
                    (res) => {
                        console.log('批量下载文件' , res);
                    }
                )
            },
            // 批量下载图片
            downloadpictures(){
                downloadpictures(this.editStatus).then(
                    (res) => {
            ...
            // 批量下载文本
            downloadtext(){
                downloadtext(this.editStatus).then(
                    (res) => {
                        console.log('批量下载文本' , res);
                    }
                )
            },
        }   
